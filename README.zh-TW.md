...
> statusline。此 fork 把渲染熱路徑從 bash 改寫為原生 Go 執行檔，消除 Windows 上的
> MSYS 殭屍程序問題（[背景](./openspec/changes/go-renderer-core/proposal.md)）。
> 尚未移植的功能仍由 bash 版提供。
...
`themes/*.conf`——切換 renderer 不需改設定。

量表會隨用量變色：綠色 → 50% 轉黃 → 75% 轉紅（門檻可調）。

## 安裝

完整安裝指南見 [INSTALL.md](./INSTALL.md)。主要路徑是建構 Go renderer：
...
- **裝完之後跑的是什麼：** Go renderer 是一個原生二進位檔，不打任何網路請求；
  唯一 spawn 的外部程序是每次 render 一個 `git` 呼叫。bash 版是純 bash，每次 render
  一個 `jq` 加一個 `git`。你的對話、金鑰、用量資料不會離開這台機器。
...
| 變數 | 預設值 | 說明 |
|---|---|---|
| `VL_STYLE` | `pill` | `pill`：powerline 膠囊 · `lean`：純色文字（僅 bash）· `classic`：文字鋪在統一深色橫條上（僅 bash） |
| `VL_LAYOUT` | `fixed` | `fixed`：每個 `VL_SEGMENTS*` 變數固定一行 · `auto`：響應式（僅 bash） |
...
`VL_FLOAT=1` 會讓 `statusline.sh` 在每次 render 時寫一行**純文字**讀數到
`~/.claude/coralline/float.txt`（區段來自 `VL_FLOAT_SEGMENTS`，預設
`model ctx cost`）。coralline 只做這件事——它**不附帶顯示載體**。這個檔案就是接點：
把它接到任何你想放一個「不用看 Claude Code 底部狀態列就能一瞥」的讀數的地方（終端
狀態列、tmux、menu-bar app⋯⋯）。

讀數是**純文字**（無 ANSI 色碼），因此預設偏好穩定、適合一瞥的區段，把仰賴顏色的
限額警示（`limit5h`／`limit7d`）留在底部狀態列。你仍然可以把它們加進
`VL_FLOAT_SEGMENTS`。
...
| 鍵 | 預設值 | 說明 |
|---|---|---|
| `VL_FLOAT` | `0` | `1` = 每次 render 寫 `float.txt` |
| `VL_FLOAT_SEGMENTS` | `model ctx cost` | 寫入讀數的區段（純文字、無色碼） |
| `VL_FLOAT_SEP` | `  ·  ` | 區段之間的分隔 |
| `VL_FLOAT_FILE` | `~/.claude/coralline/float.txt` | 讀數寫入位置 |

（也可以在 `configure.sh` 的 Details 選單中切換 `VL_FLOAT`。）

一個完整的 iTerm2 載體（`coralline-float` companion + 設定步驟）放在
[`example/float-display-iterm2/`](example/float-display-iterm2/)——複製到你的
dotfiles 再改就好。其他終端（tmux、WezTerm、menu-bar app⋯⋯）只要用同樣方式讀
`float.txt` 即可。
...
| 平台 | 狀態 |
|---|---|
| macOS | ✅ 支援——Go renderer 或 bash renderer |
| Linux | ✅ 支援——Go renderer 或 bash renderer |
| Windows | ✅ 支援——Go renderer 為建議路徑（原生 .exe，不依賴 MSYS） |
| Windows + Git Bash | ✅ bash renderer 在 Git Bash 與 `jq` 安裝後也可使用 |

> **Windows 提醒：** Go renderer 不需要 Git Bash 或 `jq` 即可原生運作。若使用 bash
> renderer，請安裝 [Git for Windows](https://git-scm.com/download/win)（內含 Git Bash）和 `jq`。

## 為什麼很快

Go renderer 是一個原生二進位檔：不打任何網路或 API 呼叫，每次 render 只 spawn 一個
`git` 子程序，並有 5 秒硬式看門狗。不需 `jq`、不需 bash、Windows 上不經 MSYS 層。

bash renderer 也很快：單次 `jq` 呼叫一口氣取出所有欄位，單次
